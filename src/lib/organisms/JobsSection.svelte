<script lang="ts">
  ...
  const SOURCE_STYLE: Record<string, { bg: string; fg: string; short: string }> = {
    ...
    "ats-scrapers": { bg: "#0f766e", fg: "#ffffff", short: "AT" },
...
<div class="jobs-wrap">
  ...
  {#if result && !loading}
    <div class="jobs-meta">
      ...
      {#if result.updatedAt}
        <p class="jobs-sync" title="Las ofertas se traen automáticamente de Bolsa EPN, Multitrabajos, Computrabajo, Trabajo.org, Indeed, LinkedIn y ats-scrapers">
